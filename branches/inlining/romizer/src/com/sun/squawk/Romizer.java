...
/*end[J2ME.STATS]*/
    ...
    /**
     * CHeaderFileCreator is created in callback from translator, then is used again after sym file created.
     */
    CHeaderFileCreator chfc;
    ...
    private void usage(String errMsg) {
        ...
        out.println( "   -retainMethodNames:<b> Retain the symbolic names of all methods in the suite");
        out.println("                        The default is false");
        ...
        while (argc != args.length) {
            ...
            if (arg == null || arg.charAt(0) != '-') {
                ...
                if (value.equals("true")) {
                    ...
                    usage("invalid retainMethodNames: " + value);
                    return null;
                }
...
/*end[J2ME.STATS]*/
    ...
    private void translate(Vector<String> classNames) {
        ...
        try {
            ...
            Runnable callback = suite.isBootstrap() ? new CHeaderCreator() : null;
	        translator.close(suiteType, callback);
    ...
    /**
     * Finish creating the C header file required by the C implementation of the Squawk interpreter.
     *
    ...
    private void createCHeader() throws IOException {
        ...
        if (chfc.update(symbols)) {
            generatedFiles.addElement(chfc.getFile().getAbsolutePath());
        } else {
            System.out.println(chfc.getFile().getAbsolutePath() + " is already up to date");
        }
    }
    
    /**
     * Start creating the C header file.
     *
     * Run as a callback from the translator, before translator srtips unused classes etc from the suite.
     */
    class CHeaderCreator implements Runnable {
        public void run() {
            try {
                if (suite.getParent() == null) {
                    chfc = new CHeaderFileCreator(suite, new File("vmcore/src/vm/rom.h"));
                }
            } catch (IOException ex) {
                ex.printStackTrace();
            }
