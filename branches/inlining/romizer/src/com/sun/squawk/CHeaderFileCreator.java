...
public final class CHeaderFileCreator {
    ...
    /**
     * Create the initial part of the header file.
     *
     * @param bootstrapSuite Suite
     * @param file File
     */
    public CHeaderFileCreator(Suite bootstrapSuite, File file) throws IOException {
        this.map = null; // set at update time.
        this.caw = new CharArrayWriter(file.exists() ? (int)file.length() : 0);
        this.out = new PrintWriter(caw);
        this.suite = bootstrapSuite;
        this.file = file;
        writeClassPartOfHeader();
    }
    ...
    /**
     * Generate rest of header file, then compare to old file constnts, if changed,
     * the write out new file.
     *
     * @param properties Properties
     * @return  true if the file was overwritten or created, false if not
     * @throws IOException
     */
    public boolean update(Properties properties) throws IOException {
        map = properties;
     ...
     *
     * This may be called during translation.
     */
	@SuppressWarnings("unchecked")
    private void writeClassPartOfHeader() throws IOException {
...
nextField:  for (int fid = 0; fid != fieldCount; fid++) {
    ...
    /**
     * Write the part of the header that depends on the symbol file, which is read into the properties map.
     *
     * This must be called after translation is complete.
     */
    private void writeRestOfHeader() throws IOException {
