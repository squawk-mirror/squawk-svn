...
public class SuiteCreator {
    ...
    final void usage(String errMsg, String[] args) {
        ...
        out.print(
                ...
                "    -retainMethodNames:<b> Retain the symbolic names of all methods in the suite\n" + 
                "                          The default is false\n" +
                "    -lnt                  retain line number tables\n" +
                "                          This implies -retainMethodsNames:true\n" +
    ...
    private Isolate processArgs(String[] args, Vector ignore) {
        ...
        boolean retainMethodNames = false;
        ...
        while (argc != args.length) {
            ...
            if (arg.charAt(0) != '-') {
                ...
                retainMethodNames = true;
                argVector.addElement(arg); // pass argument through to loader
            } else if (arg.equals("-lvt")) {
                MethodMetadata.preserveLocalVariableTables();
                argVector.addElement(arg); // pass argument through to loader
            } else if (arg.startsWith("-retainMethodNames:")) {
                String value = arg.substring("-retainMethodNames:".length());
                if (value.equals("true")) {
                    retainMethodNames = true;
                } else if (value.equals("false")) {
                    retainMethodNames = false;
                } else {
                    usage("invalid retainMethodNames: " + value, args);
                    return null;
                }
                 argVector.addElement(arg); // pass argument through to loader
