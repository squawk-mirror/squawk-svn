...
public final class CodeParser implements Context {
    ...
    public CodeParser(Translator translator, Method method, byte[] code, ConstantPool constantPool) {
        ...
        this.trace = Translator.TRACING_ENABLED && Translator.isTracing("classfile", method);
    ...
    /**
     * Delete the use of a local variable from the local variable table.
     *
     * @param local    the allocated local variable
     * @todo Is it safe to delete all local info for all locals with same index?
     */
    public void localVariableDeleted(Local local) {
        if (localVariableTable != null) {
            SquawkVector table = localVariableTable;
            for (Enumeration e = table.elements(); e.hasMoreElements();) {
                LocalVariableTableEntry lve = (LocalVariableTableEntry)e.nextElement();
                if (lve.getIndex() == local.getJavacIndex()) {
                    lve.killLocal(); 
                }
            }
        }
...
//System.err.println(msg);
     ...
     */
    public Error verifyError(String msg) {
    ...
    public Method parseMethodOperand(boolean isStatic, boolean invokeinterface) {
        ...
        Method result = constantPool.getResolvedMethod(index, isStatic, invokeinterface, this);
        return result;
    }
    ...
    public UninitializedObjectClass parseNewOperand() {
        ...
        if (type.isInterface() || type.isAbstract()) {
            Translator.throwInstantiationError(prefix("can't instantiate " + type));
        }
    ...
    public Klass parseClassOperand() {
        int index = cfr.readUnsignedShort("operand");
        Klass result = constantPool.getResolvedClass(index, this);
        return result;
    }
