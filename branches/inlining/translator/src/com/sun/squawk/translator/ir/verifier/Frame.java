...
class UninitializedReference extends StackElement {
    ...
    public final boolean checkKlass(Klass k) {
        if (k == getKlass())
            return true;
		//        if (param && k.isAssignableFrom(getKlass())) ( in inlining workspace IS THIS OLD OR NEW?)
...
final class Frame {
    ...
    private boolean param0IsThis; // the first parameter is "this", and should be final.
    ...
    public final Method popMethodSlot() {
        StackElement result = popElement();
        check(result instanceof StackMethodSlot, "unexpected value on stack for findslot", result);
        return ((StackMethodSlot)result).getMethod();
    }
    ...
    public final void storeparm(int index) {
        // don't allow code to set the "this" variable in a constructor
        //check(!param0IsThis || index > 0, " bad index to storeparm: " + index);
        // Note that putfield0 may be called within both instance and static methods, so the interpreter
        // can't assume that param0 is not null, so don't bother checking for stores to param0.
        // NOte also that the IRBuilder generates code to save the original "this" value, and uses it instead of any modified version.
