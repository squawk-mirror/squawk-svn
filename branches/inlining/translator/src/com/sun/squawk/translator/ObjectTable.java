...
//System.err.println("  " + i + "\t" + oc.getCounter() + "\t" + oc.getClass() + "\t" + oc.getObject());
    ...
    /**
     * Return true if method is unused and it's safe to ignore the method's constant object table.
     * @param translator
     * @param method
     * @return 
     */
    public boolean safeToDoClassMethodElim(Translator translator, Method method) {
        Assert.that((!method.isHosted() && !method.isAbstract() && !method.isNative()));
        return safeToDoDeadMethodElim && !translator.dme.isMarkedUsed(method);
    }
    ...
    void mergeMethodsObjectTable(Translator translator, Code[] methodsCode, boolean isStatic) {
        for (int i = 0; i < methodsCode.length; i++) {
            ...
            if (!method.isHosted() && !method.isAbstract() && !method.isNative()) {
                ...
                if (safeToDoClassMethodElim(translator, method)) {
    ...
    public void printTable() {
        System.out.println("Object Table for " + definedClass);
        Enumeration e = objectTable.elements();
        while (e.hasMoreElements()) {
