...
/*end[TYPEMAP]*/
    ...
    InstructionEmitter(IR ir, ClassFile classFile, Method method, int clearedSlots) {
        ...
        this.trace        = Translator.TRACING_ENABLED && Translator.isTracing("emitter", method);
...
/*end[FLOATS]*/
    ...
    private void checkMethodCallable(Method callee) {
        ...
        if (offset == Klass.ILLEGAL_METHOD_OFFSET) {
            Translator.throwNoSuchMethodError("Call to hosted or other no longer available method: " + Klass.toString(callee, false) + " in " + Klass.toString(method, false));
        }
        ...
        if (klass.getState() >= Klass.STATE_CONVERTED && !callee.isAbstract()) {
            ...
            if (m == null || Klass.isMissingMethodObject(m, callee.isStatic())) {
                if (Translator.FORCE_DME_ERRORS) {
                    ...
                    Translator.throwNoSuchMethodError("Call to deleted method: " + Klass.toString(callee, false) + " in " + Klass.toString(method, false));
                }
            }
        }
    }
...
/*end[FLOATS]*/
    ...
    private void invokeNative(String name, Klass type) {
        ...
        if (identifier == -1) {
            ...
            if (!VM.isHosted()) {
                // Cannot throw exception here as TCK will fail
