...
public final class ClassFileLoader implements Context {
    ...
    public void load(ClassFile cf) {
        ...
        this.traceClassInfo = Translator.TRACING_ENABLED && Translator.isTracing("classinfo", klass);
    ...
    private void load(String fileName, InputStream is) {
         ...
         * Write trace message
         */
        if (Translator.TRACING_ENABLED && Translator.isTracing("loading", klass)) {
    ...
    private Klass loadClassInfo() {
        ...
        if (thisClass != klass) {
             ...
             *   subclasses.
             */
             Translator.throwNoClassDefFoundError(prefix("'this_class' indicates wrong type: " + thisClass + " is not " + klass));
        }
        ...
        if (superIndex != 0) {
            ...
            if (Modifier.isSuitePrivate(superClass.getModifiers()) && !Isolate.currentIsolate().getLeafSuite().contains(superClass)) {
                Translator.throwLinkageError(prefix(" has a superclass " + superClass + " which is a suite-private class from another suite."));
            }
