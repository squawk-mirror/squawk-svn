...
public final class StackMerge extends StackProducer {
    ...
    /**
     * Replace one stack producer with another, if it the merge contains the oldProducer.
     *
     */
    public void replaceProducer(StackProducer oldProducer, StackProducer newProducer) {
        Assert.that(newProducer != this);
        Assert.that(newProducer != oldProducer);
        /* Some tck tests generate stack merges without producers (result of phi is implicitly popped?
         * Assert.that(producers.length !=0 , "No producers for " + this); 
         */
        
        for (int i = 0; i != producers.length; ++i) {
            if (producers[i] == oldProducer) {
                producers[i] = newProducer;
// System.out.println("replaceProducer: replacing" + oldProducer + " with " + newProducer);
                if (newProducer.isSpilt()) {
                    spill(newProducer.getSpillLocal());
                } else if (isSpilt()) {
                    newProducer.spill(getSpillLocal());
                }
                return;
            }
        }
