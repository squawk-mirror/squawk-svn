...
public final class MethodDB {
    ...
    public static final class Entry {
        ...
        /**
         * Vector of all actuall calls by this method. Does NOT include calls that were inlined.
         */
        ...
        public void addCall(Entry calledMethod) {
            ...
            if (!calls.contains(calledMethod)) {
 ...
 /*       public boolean equals(Object obj) {
            ...
            Assert.that(!methodsEqual(this.m, ((Entry)obj).m));
            return false;
        }
    ...
    public MethodDB.Entry lookupMethodEntry(Method m) {
        ...
        if (value == null) {
            value = new Entry(m, Klass.toString(m, false));
    ...
    public boolean isExternallyVisible(MethodDB.Entry mw) {
      ...
      */
     public boolean isNeverOverriden(MethodDB.Entry mw) {
         Assert.that(!mw.m.isStatic());        // caller should check
         Assert.that(!mw.m.isFinal());         // caller should check
         Assert.that(!mw.m.isPrivate());       // caller should check
         Assert.that(!mw.m.getDefiningClass().isFinal());     // caller should check
   ...
   /**
     ...
     */
    private static boolean methodsEqualSLOW(Method m1, Method m2) {
        if (m1 == m2) {
            return true;
        }
        if (m1 == null || m2 == null) {
    ...
    public static boolean methodsEqual(Method m1, Method m2) {
        if (m1 == m2 ||  m1.equals(m2)) {
            Assert.that(methodsEqualSLOW(m1, m2));
            return true;
        }
        Assert.that(! methodsEqualSLOW(m1, m2));
        return false;
