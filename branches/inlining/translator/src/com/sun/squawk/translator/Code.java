...
public final class Code {
     ...
     */
    public boolean doingInlining = false;
    
    public Method getMethod() {
        return codeParser.getMethod();
    }
     ...
     */
    private void convertPhase1(Translator translator, Method method) {
        try {
             ...
             * Write trace message.
             */
            if (Translator.TRACING_ENABLED && Translator.isTracing("converting", method)) {
     ...
     */
    private MethodBody convertPhase2(Translator translator, Method method) {
        try {
            ...
            if (Arg.get(Arg.DEAD_METHOD_ELIMINATION).getBool() &&
                ...
                Translator.numMethodsUncalled++;
                if (translator.methodDB.lookupMethodEntry(method).wasInlined()) {
             ...
             * Write trace message.
             */
            if (Translator.TRACING_ENABLED && Translator.isTracing("ir0", method)) {
                Tracer.traceln("");
             ...
             */
            if (Translator.TRACING_ENABLED && Translator.isTracing("ir1", method)) {
                Tracer.traceln("");
             ...
             * bytecode offsets.
             */
            if (Translator.TRACING_ENABLED && Translator.isTracing("ir2", method)) {
                Tracer.traceln("");
             ...
             */
            if (Translator.TRACING_ENABLED && Translator.isTracing("methods", method)) {
                Tracer.traceln("");
     ...
     */
    private void convert0(Translator translator, Method method, int phase, final Vector bodies) {
        try {
            if (phase == 1) {
                convertPhase1(translator, method);
            } else {
                MethodBody b = convertPhase2(translator, method);
                if (bodies != null)
                    bodies.addElement(b);
            }
        } catch (Error e) {
            ...
            System.err.println("Runtime exception while converting " + method + ": " + e);
            throw e;
        }
    }
