...
public final class IRBuilder {
    ...
    private void parseInstructions(Translator translator, boolean copyThis) {
        final boolean trace = Translator.TRACING_ENABLED && Translator.isTracing("jvmverifier", method);
...
/*end[FLOATS]*/
    ...
    private void opc_if(Klass type, int opcode, Target target) {
        ...
        if (tail instanceof ComparisonOp) {
            ...
            if (instruction.isLCMP()) {
                ...
                if (false /* this should work, but getting " size of recorded and derived stack differs" from processBasicBlockDelimiter below. */
                        && instruction.getRight() instanceof Constant && ((Constant)instruction.getRight()).isDefaultValue()) {
                    // compare to zero, so use opc_if after all:
                    value = instruction.getLeft();
                    
                    // readjust stack: remove result of LCMP (INT), add orgianl left value (LONG)
                    //StackProducer oldTOS = frame.pop(instruction.getType());
                    //Assert.always(oldTOS == instruction);
                    Assert.always(value.getType() == Klass.LONG);
                    frame.push(value);
                    
                    ir.remove(instruction.getRight());
    ...
    private void opc_tryend(Target target) {
        ...
        //Assert.that(handler.getTarget() == target); (where did this assert come from?)
    ...
    private void opc_return(Klass returnType) {
        ...
        if (method.getReturnType() == Klass.VOID || isConstructor) {
            ...
            if (isConstructor) {
                ...
                if (frame.getStackSize() != 0) {
                    ir.setNonEmptyStackOnReturn();
                }
            } else {
                /*
                 * Some tck tests (javasoft.sqe.tests.vm.instr.getstatic.getstatic005.getstatic00502m1_1.getstatic00502m1_1_wrapper),
                 * leave something on the stack, then do a return_v. So clean up
                 */
        ...
        Return instruction = new Return(value, frame.getStackSize());
        append(instruction);
    ...
    private void opc_getstatic(Field field) {
        if (field.hasConstant()) {
            if (VM.isHosted()) {
                // This should *never* happen as javac should inline all accesses to constants
                // and only code generated by javac should be romized
                throw Assert.shouldNotReachHere("access to constant field via getstatic not expected");
            } else {
                if (!GC.inRam(field.getDefiningClass())) {
    ...
    private void opc_putstatic(Field field) {
        if (field.hasConstant() && field.getType().isPrimitive()) {
            ...
            if (VM.isHosted()) {
                // This should *never* happen as javac should inline all accesses to constants
                // and only code generated by javac should be romized
                throw Assert.shouldNotReachHere("access to constant field via putstatic not expected");
            } else {
                if (!GC.inRam(field.getDefiningClass())) {
