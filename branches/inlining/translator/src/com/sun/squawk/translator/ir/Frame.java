...
public final class Frame {
    ...
    public Frame(CodeParser codeParser, int extraLocals) {
        ...
        if (Translator.shouldDoInlining()) {
            localRecords = new SquawkHashtable();
        } else {
            localRecords = null;
        }
    }
    ...
    private final SquawkHashtable localRecords;
    ...
    public void saveLocals(Instruction insn) {
        if (Translator.shouldDoInlining()) {
            Klass locals[] = new Klass[localTypes.length];
            System.arraycopy(localTypes, 0, locals, 0, locals.length);
            localRecords.put(insn, locals);
        }
    }
    
    public Klass[] getLocals(Instruction insn) {
        return (Klass[])localRecords.get(insn);
    }
...
//                }
    ...
    public void push(StackProducer producer) {
         ...
         */
        if (sp == maxStack) {
            throw codeParser.verifyError("operand stack overflow pushing " + producer);
        }
        stack[sp++] = producer;
        ...
        if (type.isDoubleWord()) {
            if (sp == maxStack) {
                throw codeParser.verifyError("operand stack overflow pushing doubleword");
            }
            stack[sp++] = null;
        }
    }
    ...
    /**
     * Allocate temp local fpr spilling  or inlined parameters
     *
     * @param type the type of the local
     * @return a unique local to be used to hold the value
     */
    public Local allocateLocalForTemp(Klass type) {
