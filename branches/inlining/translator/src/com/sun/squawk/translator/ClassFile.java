...
//System.err.println("    "+list[i]);
     ...
     *
     * @param ir the ir to search
     */
    public void collectConstantObjects(IR ir) {
     ...
     * Add the object references into the table of constants.
     *
     * Due to the way that vm2C handles java strings, we need to collect String constants even for "dead" methods,
     * since although the java method is not called, the C version may be.
     *
     * @param ir the ir to search
     */
    public void collectConstantStrings(IR ir) {
        for (Instruction instruction = ir.getHead() ; instruction != null ; instruction = instruction.getNext()) {
            Object object = instruction.getConstantObject();
     ...
     * @param ir the ir to search
     */
    public void collectConstantNonStrings(IR ir) {
        for (Instruction instruction = ir.getHead() ; instruction != null ; instruction = instruction.getNext()) {
            Object object = instruction.getConstantObject();
            if (object != null && !(object instanceof String)) {
                addConstantObject(object);
