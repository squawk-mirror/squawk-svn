    ...
    private void convertMethods(Translator translator, boolean isStatic, int phase, Vector bodies) {
        ...
        for (int i = 0 ; i < methodsCode.length ; i++) {
            ...
            if (method.isAbstract() && (phase == 0 || phase == 1)) {
                translator.methodDB.recordMethod(method, 0, i);
            }
            
            if (!method.isHosted() && !method.isAbstract() && !method.isNative()) {
                Assert.that(code != null);
                if (phase == 0 || phase == 1) {
                    code.convert(translator, method, 1, null);
                    int size = 0;
                    if (Translator.TRACING_ENABLED) {
                        size = code.getIR().size();
                    }
                    translator.methodDB.recordMethod(method, size, i);
                }
                
                if (phase == 0) {
                    code.postOptAccounting(translator, method);
                }
                
                if (phase == 0 || phase == 2) {
                    code.convert(translator, method, 2, bodies);
...
//}
    ...
    public void convertPhase1_5(Translator translator, boolean isStatic) {
        Code[] methodsCode = isStatic ? staticMethods : virtualMethods;
        for (int i = 0 ; i < methodsCode.length ; i++) {
            Method method = definedClass.getMethod(i, isStatic);
            Code code = methodsCode[i];
    ...
    /**
     * After inlining, dead method detection, etc, start accounting for things as they will really be in generated code.
     * @param translator 
     */
    public void convertPhase1_5(Translator translator) {
        convertPhase1_5(translator, true);
        convertPhase1_5(translator, false);
    ...
    void convertPhase1(Translator translator, boolean generateIR) {
         ...
         * Write trace message
         */
        if (Translator.TRACING_ENABLED && Translator.isTracing("converting", definedClass)) {
