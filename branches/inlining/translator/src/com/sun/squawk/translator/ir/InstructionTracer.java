...
public class InstructionTracer implements InstructionVisitor {
    ...
    public InstructionTracer(IR ir) {
        this.ir = ir;
        int addr = Math.max(getAddressOf(ir.getTail()), 10);
        indent = createSpaces(addr, 2);
        String i2 = createSpaces(Instruction.OFFSETNOTDEFINED, 2);
...
//                || (instruction instanceof Phi && ((Phi)instruction).getTarget().isBackwardBranchTarget() == false)
    ...
    public void trace(Instruction instruction) {
        ...
        /*
         * indent to indicate leaving exception handler/inlined method scope
         */
        if (instruction instanceof TryEnd ||
            instruction instanceof InlinedEnd) {
            indent = indent.substring(4);
            Assert.that(indent.length() >= 4, "indent len: " + indent.length() + " after " + instruction);
        }
        ...
        if (instruction instanceof Try ||
            instruction instanceof InlinedInvoke) {
            indent += "    ";
        }
    }
   ...
   private String traceToString(Instruction instruction, String prefix) {
    ...
    private int getAddressOf(Object object) {
        ...
        while (address == Instruction.OFFSETNOTDEFINED) {
            ...
            if (instruction == null) {
                return Instruction.OFFSETNOTDEFINED;
            } else {
...
/*end[FLOATS]*/
    ...
    public void doInlinedInvoke(InlinedInvoke instruction) {
        out.append("// >>> Start inline of " + instruction.getMethod());
    }
    ...
    public void doInlinedEnd(InlinedEnd instruction) {
        out.append("// <<< End inline of " + instruction.getMethod());
    }
