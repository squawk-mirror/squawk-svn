...
public final class IRTransformer implements OperandVisitor {
    ...
    public void transform(Translator translator) {
        boolean recordCalls = (translator.getTranslationStrategy() >= Translator.BY_SUITE) && !translator.shouldOptimizeSuite(); // if inlining, record method calls after inlining.
         ...
         * Write trace message.
         */
        if (Translator.isTracing("transform", method)) {
            Tracer.traceln("");
            Tracer.traceln("++++ Transform(before spills added)  for " + method + " ++++");
      ...
      //  ir.verify(false);
        ...
        while (instruction != null) {
            if (instruction instanceof StackProducer) {
                ...
                if (producer.isSpilt()) {
                    ...
                    store.setBytecodeOffset(instruction.getBytecodeOffset());
        ...
        if (Translator.isTracing("transform", method)) {
            Tracer.traceln("");
            Tracer.traceln("++++ Transform(after spills added)  for " + method + " ++++");
            new InstructionTracer(ir).traceAll();
            Tracer.traceln("---- Transform(after spills added) for " + method + " ----");
        }
    }
    ...
    /**
     * As part of translating dups and filling, be careful to replace only one use of a stack producer with the new fill value.
     * Note that doOperand() does this implicitly. All other callers of fill() must call replaceOneOperand().
     *
     * Consider something like:
     * 
     * load_param1                      load_param1
     * dup                              store tmp1
     * dup                          =>  load tmp1
     * invoke_static foo(int, int):V    load tmp1
     * invoke_static bar(int):V         invoke_static foo(int, int) 
     *                                  load tmp1
     *                                  invoke_static bar(int):V
     *
     * @param newop the fill for the spilled operand
     * @param oldOp the spilled operand
     * @param instruction the instruction that consumes the spilled operand
     */
    private void replaceOneOperand(StackProducer newOp, StackProducer oldOp, Instruction instruction) {
        OperandVisitor visitor = new OperandReplacer(newOp, oldOp, true);
    ...
    private StackProducer fill(Instruction instruction, StackProducer operand) {
        ...
        load.setBytecodeOffset(instruction.getBytecodeOffset()); // attach fill to the consumer instruction
    ...
    private void fillReversedParameters(Instruction instruction, StackProducer operand) {
        if (!canReplaceSpillWithUse(instruction, operand)) {
            StackProducer newLoad = fill(instruction,  operand);
            replaceOneOperand(newLoad, operand, instruction);
        }
    }
