...
public final class ConstantObject extends Constant {
    ...
    /**
     * Return true if the constant is an instance of klass.
     * Note: Had trouble using Klass.isInstanceOf(getConstantObject()),
     * becuase the constant value is a HotSpot object??? For now,
     * rely on a match between the declared type of the ConstantObject with the value
     * (see constructor).
     *
     * @param klass the klass to test with
     * @return true if the constant is an instance of klass.
     */
    public boolean isInstanceOf(Klass klass) {
        Assert.that(value != null); // caller must handle.
        return klass.isAssignableFrom(this.getType());
    }
